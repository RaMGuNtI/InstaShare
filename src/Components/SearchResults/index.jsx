...
const SearchResults = ({ serres }) => {
  const postDetails = serres;
  const { inp } = useParams();
  ...
  useEffect(() => {
    const fetchSearch = async () => {
      const response = await fetch(
        `https://apis.ccbp.in/insta-share/posts?search=${inp}`,
        options
      );
      const data = await response.json();
      setGetSearch(data.posts);
      console.log(data);
    };
    fetchSearch();
  }, []);
  ...
  if (getSearch !== null && getSearch.length == 0) {
    return (
      ...
      >
        <img src="https://res.cloudinary.com/dez8wfcvn/image/upload/v1751859317/Group_1_uv2elc.png" />
            ...
            return (
              <div>
                ...
                <div className="postrelateddetails">
                  <p>{postDetails.likes_count} likes</p>
